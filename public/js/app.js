$(document).ready(function () {
    ...
    $(document).on('click', '.btn-modal', function (e) {
        ...
        // Normalize ai-template links when user is in default (non-ai) URL space.
        try {
            var isAiTemplatePage = window.location && window.location.pathname && window.location.pathname.indexOf('/ai-template') === 0;
            if (!isAiTemplatePage) {
                if (href.indexOf('/ai-template') === 0) {
                    href = href.replace(/^\/ai-template/, '');
                    if (href === '') {
                        href = '/home';
                    }
                } else if (window.location && window.location.origin && href.indexOf(window.location.origin + '/ai-template') === 0) {
                    href = href.replace(window.location.origin + '/ai-template', window.location.origin);
                }
            }
        } catch (e) {
            // no-op: keep original href
        }
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if ($container.length === 0) {
                    ...
                    if (container[0] === '#') {
                        $fallback.attr('id', container.slice(1));
                    } else if (container[0] === '.') {
                        $fallback.addClass(container.slice(1));
                    } else {
                        $fallback.addClass('view_modal');
                    }
    ...
    // Keep default pages from jumping into ai-template via plain anchor links.
    $(document).on('click', 'a', function (e) {
        var $link = $(this);
        if ($link.data('allow-ai-template') || $link.hasClass('allow-ai-template')) {
            return;
        }
        ...
        var normalized = null;
        if (href.indexOf('/ai-template') === 0) {
            normalized = href.replace(/^\/ai-template/, '');
            if (normalized === '') {
                normalized = '/home';
            }
        } else if (window.location && window.location.origin && href.indexOf(window.location.origin + '/ai-template') === 0) {
            normalized = href.replace(window.location.origin + '/ai-template', window.location.origin);
        }
    ...
    //Brands table
    var brands_ajax_url = (window.location && window.location.pathname && window.location.pathname.indexOf('/ai-template') === 0)
        ? '/ai-template/brands'
        : '/brands';
    ...
    //Start: CRUD for unit
    //Unit table
    var units_ajax_url = (window.location && window.location.pathname && window.location.pathname.indexOf('/ai-template') === 0)
        ? '/ai-template/units'
        : '/units';
    ...
    var contacts_ajax_url = (window.location && window.location.pathname && window.location.pathname.indexOf('/ai-template') === 0)
        ? '/ai-template/contacts'
        : '/contacts';

    if ($.fn.DataTable && $.fn.DataTable.isDataTable('#contact_table')) {
    ...
    contact_table = $('#contact_table').DataTable({
        ...
        scrollCollapse: true,
        columnDefs: [
            { targets: 0, orderable: false, searchable: false, width: "80px", className: "dt-nowrap" }
        ],
        "ajax": {
            "url": contacts_ajax_url,
            "data": function (d) {
        ...
        fnDrawCallback: function (oSettings) {
            ...
            // Viho only: replace $ with ৳ (UI requirement).
            if (window.location && window.location.pathname && window.location.pathname.indexOf('/ai-template') === 0) {
                try {
                    $('#contact_table').find('*').each(function () {
                        var $el = $(this);
                    ...
                    $('.footer_contact_due, .footer_contact_return_due').each(function () {
                        var txt = $(this).text();
                        if (txt && txt.indexOf('$') !== -1) {
        ...
        initComplete: function () {
            var relocate = function () {
                if ($('#contacts_dt_length').length < 1) return;
                ...
                if ($length.length) $('#contacts_dt_length').empty().append($length);
                if ($filter.length) $('#contacts_dt_filter').empty().append($filter);
                if ($info.length) $('#contacts_dt_info').empty().append($info);
                if ($paginate.length) $('#contacts_dt_paginate').empty().append($paginate);
            };
    ...
    //Variations table
    var variation_ajax_url = (window.location && window.location.pathname && window.location.pathname.indexOf('/ai-template') === 0)
        ? '/ai-template/variation-templates'
        : '/variation-templates';
    ...
    var invoice_table_url = '/invoice-schemes';
    if (window.location.pathname.indexOf('/ai-template') === 0) {
        invoice_table_url = '/ai-template/invoice-schemes';
    }
    var invoice_table = $('#invoice_table').DataTable({
        ...
        drawCallback: function (settings) {
            var $wrapper = $('#invoice_table_wrapper');
            if ($wrapper.length < 1) return;
            ...
            if ($length.length) $('#invoice_dt_length').empty().append($length);
            if ($filter.length) $('#invoice_dt_filter').empty().append($filter);
            if ($info.length) $('#invoice_dt_info').empty().append($info);
            if ($paginate.length) $('#invoice_dt_paginate').empty().append($paginate);
        }
    });
    ...
    if ($.fn.DataTable && $.fn.DataTable.isDataTable('#sales_commission_agent_table')) {
    ...
    var is_ai_template_cg = window.location && window.location.pathname && window.location.pathname.indexOf('/ai-template') === 0;
    var customer_group_ajax_url = is_ai_template_cg ? '/ai-template/customer-group' : '/customer-group';

    if ($.fn.DataTable && $.fn.DataTable.isDataTable('#customer_groups_table')) {
        $('#customer_groups_table').DataTable().destroy();
        $('#customer_groups_table').find('tbody').remove();
    }
    ...
    var customer_groups_table = $('#customer_groups_table').DataTable({
        ...
        drawCallback: function () {
            if (is_ai_template_cg) {
                // move DT controls into Viho placeholders (if present)
                var relocate = function () {
                    var $wrapper = $('#customer_groups_table_wrapper');
                    if ($wrapper.length < 1) return;
                    ...
                    if ($length.length) $('#customer_groups_dt_length').empty().append($length);
                    if ($filter.length) $('#customer_groups_dt_filter').empty().append($filter);
                    if ($info.length) $('#customer_groups_dt_info').empty().append($info);
                    if ($paginate.length) $('#customer_groups_dt_paginate').empty().append($paginate);
                };
                relocate();
            }
        },
