...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        // Init popovers once (avoid binding nested document click handlers that can interfere with dropdowns/select2).
        ...
        $(document).on('click', '.js-layout-template-switch', function() {
            var $btn = $(this);
            var templateKey = $btn.data('template-key');
            var updateUrl = $btn.data('update-url');

            if (!templateKey || !updateUrl) {
                return;
            }
