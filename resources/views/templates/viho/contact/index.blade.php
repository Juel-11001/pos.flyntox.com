...
<div class="container-fluid">
  <div class="page-header">
    <div class="row">
      <div class="col-sm-6">
        <h3>@lang('lang_v1.' . $type . 's') <small class="text-muted">@lang('contact.manage_your_contact', ['contacts'
            => __('lang_v1.' . $type . 's')])</small>
        </h3>
      </div>
      <div class="col-sm-6 text-right">
        ...
        <a class="btn btn-primary btn-sm btn-modal"
          data-href="{{ route('ai-template.contacts.create', ['type' => $type]) }}" data-container=".contact_modal">
        ...
        <div class="row">
          ...
          <div class="col-sm-12 col-md-6 col-xl-4 col-xxl-3">
            <div class="form-group">
              <div class="checkbox">
                <label>
                  {!! Form::checkbox('has_sell_return', 1, false, ['class' => 'input-icheck', 'id' =>
                  'has_sell_return']) !!}
          ...
          <div class="col-sm-12 col-md-6 col-xl-4 col-xxl-3">
            <div class="form-group">
              ...
              {!! Form::select('assigned_to', $users, null, ['class' => 'form-control select2', 'style' =>
              'width:100%']) !!}
            </div>
          </div>
          @endif
...
$(document).ready(function() {
  // Fix icons for Viho template - changes fa-money-bill-alt to fa-money-bill and fa-scroll to fa-book
  var fixVihoContactIcons = function() {
    $('#contact_table .dropdown-menu a i').each(function() {
      var $icon = $(this);
      // Change pay icon
      if ($icon.hasClass('fa-money-bill-alt')) {
        $icon.removeClass('fa-money-bill-alt').addClass('fa-money-bill');
      }
      // Change ledger icon
      if ($icon.hasClass('fa-scroll')) {
        $icon.removeClass('fa-scroll').addClass('fa-book');
      }
    });
  };
  ...
  contact_table = $('#contact_table').DataTable({
    ...
    "footerCallback": function(row, data, start, end, display) {
      ...
      for (var r in data) {
        footer_contact_due += $(data[r].due).data('orig-value') ? parseFloat($(data[r].due).data(
          'orig-value')) : 0;
        footer_contact_return_due += $(data[r].return_due).data('orig-value') ? parseFloat($(data[r]
          .return_due).data('orig-value')) : 0;
      }
