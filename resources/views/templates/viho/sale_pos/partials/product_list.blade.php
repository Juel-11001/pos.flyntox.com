@forelse($products as $product)
	<div class="col-md-3 col-xs-4 product_list no-print">
		...
		<div class="image-container">
			<img src="@if(count($product->media) > 0){{$product->media->first()->display_url}}@elseif(!empty($product->product_image)){{asset('/uploads/img/' . rawurlencode($product->product_image))}}@else{{asset('/img/default.png')}}@endif" alt="{{$product->name}}">
		</div>
