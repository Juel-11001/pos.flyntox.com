			...
			<div class="row tw-flex lg:tw-flex-row md:tw-flex-col sm:tw-flex-col tw-flex-col tw-items-start md:tw-gap-4">
					...
					<div class="tw-shadow-[rgba(17,_17,_26,_0.1)_0px_0px_16px] tw-rounded-2xl tw-bg-white tw-mb-2 md:tw-mb-8 tw-p-2">
						<div class="box-body pb-0">
							...
							<input type="hidden" id="item_addition_method" value="{{$business_details->item_addition_method}}">
								@include('templates.viho.sale_pos.partials.pos_form_edit')

								@include('templates.viho.sale_pos.partials.pos_form_totals', ['edit' => true])

								@include('templates.viho.sale_pos.partials.payment_modal')

								@if(empty($pos_settings['disable_suspend']))
									@include('templates.viho.sale_pos.partials.suspend_note_modal')
								@endif

								@if(empty($pos_settings['disable_recurring_invoice']))
									@include('templates.viho.sale_pos.partials.recurring_invoice_modal')
								@endif
							</div>
				...
				@if(empty($pos_settings['hide_product_suggestion'])  && !isMobile() && empty($only_payment))
					<div class="col-md-5 no-padding">
						@include('templates.viho.sale_pos.partials.pos_sidebar')
					</div>
				@endif
			</div>
		</div>
	</div>
	@include('templates.viho.sale_pos.partials.pos_form_actions', ['edit' => true])
	{!! Form::close() !!}
</section>
...
@if(empty($pos_settings['hide_product_suggestion']) && isMobile())
	@include('templates.viho.sale_pos.partials.mobile_product_suggestions')
@endif
<!-- /.content -->
...
@include('templates.viho.sale_pos.partials.configure_search_modal')

@include('templates.viho.sale_pos.partials.recent_transactions_modal')

@include('templates.viho.sale_pos.partials.weighing_scale_modal')
...
@section('javascript')
	...
	@include('templates.viho.sale_pos.partials.keyboard_shortcuts')
