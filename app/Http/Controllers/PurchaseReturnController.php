...
class PurchaseReturnController extends Controller
{
    ...
    public function index()
    {
        ...
        if (request()->ajax()) {
            ...
            return Datatables::of($purchases_returns)
                ->addColumn('action', function ($row) {
                    $is_viho = request()->is('ai-template/*') || (method_exists($this, 'isAiTemplateRequest') && $this->isAiTemplateRequest());
                    $edit_url = $is_viho
                        ? route('ai-template.purchase-return.add', [$row->return_parent_id ?: $row->id])
                        : action([\App\Http\Controllers\PurchaseReturnController::class, 'add'], $row->return_parent_id ?: $row->id);
                    $delete_url = $is_viho
                    ...
                    if (! empty($row->return_parent_id)) {
                        $html .= '<li><a href="'.$edit_url.'" ><i class="glyphicon glyphicon-edit"></i>'.
                                __('messages.edit').
                                '</a></li>';
                    } else {
                ...
                ->editColumn('parent_purchase', function ($row) {
                    ...
                    if (! empty($row->parent_purchase)) {
                        ...
                        $href = $is_ai_template ? url('/ai-template/purchases/'.$row->return_parent_id) : url('/purchases/'.$row->return_parent_id);
                        $html = '<a href="#" data-href="'.$href.'" class="btn-modal" data-container=".view_modal">'.$row->parent_purchase.'</a>';
                    }
                ...
                ->setRowAttr([
                    'data-href' => function ($row) {
                        if (auth()->user()->can('purchase.view')) {
                            ...
                            return request()->is('ai-template/*')
                                ? route('ai-template.purchase-return.show', [$return_id])
                                : action([\App\Http\Controllers\PurchaseReturnController::class, 'show'], [$return_id]);
                        } else {
                            return '';
                        }
                    }, ])
        ...
        return view($this->viewPath('index'))->with(compact('business_locations'));
    }
