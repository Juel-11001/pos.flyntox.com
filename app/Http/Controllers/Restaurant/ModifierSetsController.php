        ...
        if (request()->ajax()) {
            ...
            return \Datatables::of($modifer_set)
                ->addColumn(
                    'action',
                    function ($row) {
                        $is_viho = request()->segment(1) === 'ai-template';
                        $edit_url = $is_viho
                            ? route('ai-template.modifiers.edit', [$row->id])
                            : route('modifiers.edit', [$row->id]);
                        $delete_url = $is_viho
                            ? route('ai-template.modifiers.destroy', [$row->id])
                            : route('modifiers.destroy', [$row->id]);
                        $manage_url = url('product-modifiers/' . $row->id . '/edit');

                        $html = '';
                        if (auth()->user()->can('product.update')) {
                            $html .= '<button type="button" data-href="' . $edit_url . '" class="tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline tw-dw-btn-primary edit_modifier_button" data-container=".modifier_modal"><i class="glyphicon glyphicon-edit"></i> ' . __('messages.edit') . '</button>
                                &nbsp;
                                <button type="button" data-href="' . $manage_url . '" class="tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline tw-dw-btn-info edit_modifier_button" data-container=".modifier_modal"><i class="fa fa-cubes"></i> ' . __('restaurant.manage_products') . '</button>&nbsp;';
                        }

                        if (auth()->user()->can('product.delete')) {
                            $html .= '<button data-href="' . $delete_url . '" class="tw-dw-btn tw-dw-btn-outline tw-dw-btn-xs tw-dw-btn-error delete_modifier_button"><i class="glyphicon glyphicon-trash"></i> ' . __('messages.delete') . '</button>';
                        }
