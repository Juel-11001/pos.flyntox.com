            ...
            return Datatables::of($tables)
                ->addColumn(
                    ...
                    function ($row) use ($business_id) {
                        ...
                        $edit_url = $is_viho
                            ? route('ai-template.tables.edit', [$row->id])
                            : route('tables.edit', [$row->id]);
                        $delete_url = $is_viho
                            ? route('ai-template.tables.destroy', [$row->id])
                            : route('tables.destroy', [$row->id]);

                        $html = '';
                        if (auth()->user()->hasRole('Admin#' . $business_id)) {
                            $html .= '<button data-href="' . $edit_url . '" class="tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline tw-dw-btn-primary edit_table_button"><i class="glyphicon glyphicon-edit"></i> ' . __('messages.edit') . '</button>&nbsp;';
                            $html .= '<button data-href="' . $delete_url . '" class="tw-dw-btn tw-dw-btn-outline tw-dw-btn-xs tw-dw-btn-error delete_table_button"><i class="glyphicon glyphicon-trash"></i> ' . __('messages.delete') . '</button>';
                        }
