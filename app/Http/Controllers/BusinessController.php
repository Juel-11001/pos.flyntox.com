...
class BusinessController extends Controller
{
    ...
    */
    ...
    {
        return 'templates.viho.business.' . $view;
    }
    ...
    public function __construct(BusinessUtil $businessUtil, RestaurantUtil $restaurantUtil, ModuleUtil $moduleUtil)
    {
        ...
        $this->layoutTemplates = config('constants.layout_templates', [
            'default' => 'Default',
        ]);
    }
    ...
    public function getBusinessSettings()
    {
        ...
        // Layout templates for template selection
        $layout_templates = $this->layoutTemplates;

        // Use Viho template view if request is for AI template
        if ($this->isAiTemplateRequest()) {
            return view($this->viewPath('settings'), compact('business', 'currencies', 'tax_rates', 'timezone_list', 'months', 'accounting_methods', 'commission_agent_dropdown', 'units_dropdown', 'date_formats', 'shortcuts', 'pos_settings', 'modules', 'theme_colors', 'email_settings', 'sms_settings', 'mail_drivers', 'allow_superadmin_email_settings', 'custom_labels', 'common_settings', 'weighing_scale_setting', 'payment_types', 'layout_templates'));
        }
    ...
    /**
     * Update active layout template for the business.
     *
     * @return \Illuminate\Http\JsonResponse
     */
    public function updateLayoutTemplate(Request $request)
    {
        if (! auth()->user()->can('business_settings.access')) {
            abort(403, 'Unauthorized action.');
        }
