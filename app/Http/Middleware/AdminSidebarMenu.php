...
class AdminSidebarMenu
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
            ...
            // Use saved layout_template to decide which template menu to render.
            $layout_template = $common_settings['layout_template'] ?? 'default';
            $is_ai_template = is_string($layout_template) && strtolower($layout_template) === 'viho';
            //Home
                    ...
                    function ($sub) use ($enabled_modules, $is_admin, $pos_settings, $is_ai_template) {
                        ...
                        if (in_array('subscription', $enabled_modules) && auth()->user()->can('direct_sell.access')) {
                            ...
                            $sub->url(
                                ...
                                ['icon' => '', 'active' => ($is_ai_template && request()->segment(2) == 'sells' && request()->segment(3) == 'subscriptions') || (request()->segment(1) == 'sells' && request()->segment(2) == 'subscriptions')]
                            );
                        }
            ...
            if (in_array('expenses', $enabled_modules) && (auth()->user()->can('all_expense.access') || auth()->user()->can('view_own_expense'))) {
                $menu->dropdown(
                    __('expense.expenses'),
                    function ($sub) use ($is_ai_template) {
                        $expense_list_url = $is_ai_template ? '/ai-template/expenses' : route('expenses.index');
                        $sub->url(
                            $expense_list_url,
                        ...
                        if (auth()->user()->can('expense.add')) {
                            $expense_create_url = $is_ai_template ? '/ai-template/expenses/create' : route('expenses.create');
                            $sub->url(
                                $expense_create_url,
                        ...
                        if (auth()->user()->can('expense.add') || auth()->user()->can('expense.edit')) {
                            $expense_category_url = $is_ai_template ? '/ai-template/expense-categories' : route('expense-categories.index');
                            $sub->url(
